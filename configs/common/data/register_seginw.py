...
def get_metadata(anno_path):
    with open(anno_path, 'r') as f:
        anno = json.load(f)
    thing_ids = [k["id"] for k in anno["categories"]]
...
def get_openseg_labels(dataset_name):
    category_list = SEGINW_CATEGORY_NAMES[dataset_name]
    return [[c.lower()] for c in category_list]
...
def register_all_seginw(root):
    for (
        ...
        metadata = get_metadata(os.path.join(root, folder_name, split, annot_name))
        register_seginw(
            prefix,
            metadata,
        ...
        SEGINW_CATEGORY_NAMES[prefix] = metadata["thing_classes"]
